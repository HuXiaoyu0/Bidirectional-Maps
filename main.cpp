...
void StudentTests() {
  ...
  values.push_back(32.5);
  values.push_back(13.7);
  values.push_back(14.8);
  values.push_back(43.4);
  ...
  // ------------------------------------------------
  // CREATE A BIDIRECTIONAL MAP
  bidirectional_map<char,float> bdmap;
  assert (bdmap.size() == 0);
  ...
  // sorted versions of the input data for testing the iterators
  std::vector<char> sorted_keys = keys;
  std::sort(sorted_keys.begin(),sorted_keys.end());
  ...
  // FORWARD ITERATOR ON KEYS
  int counter = 0;
  for (bidirectional_map<char,float>::key_iterator ki = bdmap.key_begin();
       ki != bdmap.key_end(); ki++) {
    assert (sorted_keys[counter] == (*ki).first);
    std::cout << "key:" << (*ki).first << " associated with value:" << (*ki).second << std::endl;
    counter++;
  ...
  // FORWARD ITERATOR ON VALUES
  counter = 0;
  for (bidirectional_map<char,float>::value_iterator vi = bdmap.value_begin();
       vi != bdmap.value_end(); vi++) {
    assert (sorted_values[counter] == (*vi).first);
    std::cout << "value:" << (*vi).first << " associated with key:" << (*vi).second << std::endl;
    counter++;
  ...
  // BACKWARD ITERATOR ON KEYS
  counter = 5;
  bidirectional_map<char,float>::key_iterator ki = bdmap.find(sorted_keys[counter]);
  while (1) {
    assert ((*ki).first == sorted_keys[counter]);
    if (counter == 0) break;
    counter--;
  ...
  // BACKWARD ITERATOR ON VALUES
  counter = 5;
  bidirectional_map<char,float>::value_iterator vi = bdmap.find(sorted_values[counter]);
  while (1) {
    assert ((*vi).first == sorted_values[counter]);
    if (counter == 0) break;
    counter--;
  ...
  bidirectional_map<char,float> bdmap_copy(bdmap);
  assert (bdmap_copy.size() == 6);
  bdmap_copy.print(std::cout);
  for (int i = 0; i < 6; i++) {
    assert (bdmap_copy[keys[i]] == values[i]);
    assert (bdmap_copy[values[i]] == keys[i]);
  }

  std::cout<< "copy constructor tested" <<std::endl;
  
  //Test the assignment operator
  bidirectional_map<char,float> bdmap_assign = bdmap;
  assert (bdmap_assign.size() == 6);
  for (int i = 0; i < 6; i++) {
    assert (bdmap_assign[keys[i]] == values[i]);
    assert (bdmap_assign[values[i]] == keys[i]);
  }
  bdmap_assign.print(std::cout);
  
  std::cout<<"assignment operator tested"<<std::endl;

  //test all corner cases with insert, find, and erase
  
  std::pair<bidirectional_map<char,float>::key_iterator,bool> insert_return;
  insert_return = bdmap.insert(std::make_pair('p',44));
  assert (insert_return.first == bdmap.find('p'));
  assert (insert_return.second == true);
  insert_return = bdmap.insert(std::make_pair('f',17));
  assert (insert_return.first == bdmap.find('f'));
  assert (insert_return.second == false);
  std::cout<<"JJ"<<bdmap.size()<<std::endl;
  assert (bdmap.size() == 7);
  std::cout << "return value of insert correct" << std::endl;

  assert (bdmap.size() == 7);
  int num_erased = bdmap.erase('q');
  assert (num_erased == 0);
  assert (bdmap.size() == 7);
  bdmap.print(std::cout);
  std::cout << "erased key:'zebra' and value:26" << std::endl;
  num_erased = bdmap.erase('f');
  assert (num_erased == 1);
  assert (bdmap.size() == 6);
  bdmap.print(std::cout);
  std::cout << "erased key:'carrot' and value:4" << std::endl;
  num_erased = bdmap.erase('d');
  assert (num_erased == 1);
  assert (bdmap.size() == 5);
  bdmap.print(std::cout);
  std::cout << "erased key:'date' and value:6" << std::endl;
  num_erased = bdmap.erase('x');
  assert (num_erased == 1);
  assert (bdmap.size() == 4);
  ...
  std::cout<<"all corner cases of insert, find, and erase tested"<<std::endl;
  std::cout << "bidirectional_maps with other data types tested"<<std::endl;
